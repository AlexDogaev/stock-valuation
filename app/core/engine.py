...
def evaluate_issuer(db: sqlite3.Connection, secid: str, macro_frag: dict | None = None) -> dict[str, Any] | None:
    ...
    eff_real_base = at_real if tax_aware else fr.real      # базовая реал. (шок-просадки нет; де-рейтинг пылесоса —
    #                                                        НЕ здесь: сигнал уже выражает мёртвый рынок бенчем=ОФЗ,
    #                                                        де-рейтинг идёт только в ПРОГНОЗ апсайда, не двойня сигнал)
    ...
    if _pe_rev_drag >= 0.005:
        warnings.append(
            ...
            f"−{_pe_rev_drag*100:.1f}%/год вычтена из ПРОГНОЗНОГО АПСАЙДА (фискальное доминирование сжимает "
            f"мультипликаторы; нет бида на ре-рейтинг вверх). Сигнал — отдельно, по бенчу=ОФЗ (без двойного счёта).")
