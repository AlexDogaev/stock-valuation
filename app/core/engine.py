...
COUNTRY_PREMIUM = 0.04      # страновая премия для РАВНОВЕСНОГО P/E (спека §3 ~4пп). Отдельно от сигнальной
#                             risk_premium (7%, бар покупки): равновесный = справедливый мультипликатор, не «мой бар».
...
def evaluate_issuer(db: sqlite3.Connection, secid: str, macro_frag: dict | None = None) -> dict[str, Any] | None:
    ...
    _eq_pe = valuation.equilibrium_pe(
        payout=valuation.MATURE_PAYOUT, ofz_nominal=_ofz_nominal, premium=COUNTRY_PREMIUM,
    ...
    # базовая реал. − де-рейтинг пылесоса (без шок-просадки). Де-рейтинг = РЕАЛЬНАЯ потеря капитала (мультипликатор
    # сжимается к равновесному) → обязан быть в ПОЛНОЙ доходности и сигнале: иначе фантом «рост при постоянном P/E»
    # делает имя ПОКУПАЙ, хотя цена прогнозно падает (кейс X5: дивы<ОФЗ + даунсайд, но +4.9% реал → ложный ПОКУПАЙ).
    eff_real_base = (at_real if tax_aware else fr.real) - _pe_rev_drag
    ...
    if _pe_rev_drag >= 0.005:
        warnings.append(
            ...
            f"−{_pe_rev_drag*100:.1f}%/год вычтена из апсайда И ПОЛНОЙ доходности (де-рейтинг = реальная потеря "
            f"капитала → и в сигнал). Фиск.доминирование сжимает мультипликаторы; нет бида на ре-рейтинг вверх.")
...
def screen_fx(db: sqlite3.Connection) -> dict:
    ...
    # КРЕДИТ ЗАМЕЩАЕК (Саша 25.06): спред над СУВЕРЕННЫМ РФ-ЗО той же валюты → PD → −PD×LGD (как у рублёвых
    # бондов). РФ-ЗО (суверен) → спред 0 → драг 0; корпорат (ГТЛК/Акрон) → положит.спред → кредит-драг.
    ...
    for b in clean:
        ...
        out.append({**b, "ytm_fx": b["ytm"], "e_fx_move": fa.e_fx_move,
                    "pd": round(pd_ann, 4), "pd_horizon": round(pd_hz, 4),     # кредит-риск (спред над сувереном)
                    "fx_spread": round(spread_fx, 4) if spread_fx is not None else None,
