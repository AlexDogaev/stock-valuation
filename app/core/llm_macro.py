...
И ВЕКТОР ШОКА — если шок (любой) реализуется, в СРЕДНЕМ (КАЛИБРУЙ по 2014 и 2022):
- imoex_drawdown_pct: УСЛОВНАЯ просадка IMOEX *если* шок, % (2014 ≈30, 2022 ≈43; это НЕ безусловная);
- recovery_1y_pct: какая ДОЛЯ просадки отыграется за 1 год, % (2014 быстро через экспортёров, 2022 частично);
...
- sectoral: просадка по секторам в шоке (доли, отриц.): экспортёры (смягчает девал), внутренние, банки, IT.
Верни СТРОГО JSON без обрамления:
{"horizon":"12 мес","aggregate_pct":int,"scenarios":[{"name":"...","prob_pct":int,"severity_pct":int,"factor":"...","rationale":"1 фраза"}],"imoex_drawdown_pct":float,"recovery_1y_pct":float,"shock_infl_pp":float,"shock_fx_pct":float,"shock_ks_pp":float,"sectoral":{"экспортёры":float,"внутренние":float,"банки":float,"IT":float},"note":"итог 2-3 фразы (раздели «вероятно но переживём» и «маловероятно но катастрофа»)"}"""
...
def get_shock(db: sqlite3.Connection) -> dict | None:
    ...
    try:
        ...
        d["scenarios"] = []
    try:
        d["sectoral"] = json.loads(d.get("sectoral_json") or "{}")
    except (json.JSONDecodeError, TypeError):
        d["sectoral"] = {}
    return d
...
def assess_shock(db: sqlite3.Connection) -> dict:
    ...
    def _f(key):  # пп/проценты → доля, безопасно
        try:
            ...
            return None
    def _raw(key):  # сырой процент (для условной просадки: _shock_vector делит /100 сам)
        try:
    ...
    upsert(db, "shock_risk", dict(
        ...
        imoex_drawdown_pct=_raw("imoex_drawdown_pct"), recovery_1y=_f("recovery_1y_pct"),
        sectoral_json=json.dumps(data.get("sectoral") or {}, ensure_ascii=False)[:600],
        model=os.environ.get("ANTHROPIC_MODEL", llm.DEFAULT_MODEL),
        created_at=datetime.now().isoformat(timespec="seconds"),
    ), pk="id")
    return get_shock(db)
