...
# felt-КОРИДОР (§6): вместо жёсткой ×2-догадки — диапазон [Росстат-низ .. ЦБ-инфО-опрос-верх]. Центр 2.0.
# Низ — Росстат занижает для горожанина; верх — опрос ЦБ ЗАВЫШАЕТ (эмоц.перекос → дисконтировать). Калибровка
# веса по личной корзине владельца — РАЗОВО, не сделано. ВАЖНО: после бенч=ОФЗ (§4) дефлятор СОКРАЩАЕТСЯ в
# сигнале (real и ofz_real оба над ним) → коридор НЕ влияет на относительное решение, только на абсолютный реал%.
ROSSTAT_RATIO_LO, ROSSTAT_RATIO_HI = 1.6, 2.4
