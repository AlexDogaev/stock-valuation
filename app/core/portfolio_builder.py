...
всего подходящих под «купил и держи», + два риск-метрика: РИСК ПОЛНОГО ОБНУЛЕНИЯ и РИСК −50% РЕАЛ.
...
LGD = 0.65
ROSSTAT_RATIO = 2.0      # личная инфляция : официальный Росстат CPI (Саша 20.06.2026): ставишь 12 → Росстат 6.
                         # Линкер индексируется на ОФИЦИАЛЬНЫЙ CPI, дефлятор реала — личная инфляция.
...
def build(db, *, horizon: int, equity_cap: float, exp_inflation: float, target_real: float,
    ...
    for b in rb.get("bonds", []):
        ...
        if b["coupon_type"] == "Линкер":
            official = exp_inflation / ROSSTAT_RATIO
    ...
    return {
        ...
        "loss50_risk": loss50_risk, "wipeout_risk": wipeout_risk,
