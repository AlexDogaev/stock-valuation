...
def equilibrium_pe(*, payout: float, ofz_nominal: float, premium: float, e_inflation: float,
    ...
    # БЕЗ −g (Саша 25.06): мультипликатор = эффект ПЫЛЕСОСА капитала (нет бида), НЕ награда за рост бизнеса.
    # Рост g живёт в АПСАЙДЕ (прибыль компаундит), не в мультипликаторе → растущее имя НЕ освобождается от
    # де-рейтинга через раздутый ростом равновесный (был баг: T g 23% → равнов. 6.9 → пропускала пылесос).
    spread = (ofz_nominal + premium + fiscal_drain) + e_inflation * (1.0 - passthrough)
